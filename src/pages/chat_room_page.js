        ...
        if (messages.length > 0 && otherUser) {
            ...
            setPageContext({
                ...
                dm_context: {
                    ...
                    instruction: `【最重要】ユーザーは「${otherUser.username}」とダイレクトメッセージ中です。${relationshipSummary}` +
                        `ユーザーが「代わりに返事して」「返信を考えて」「返信案」「メッセージを作って」など返信に関するお願いをしたら、` +
                        `絶対に「【返信案】」で始まる返信文を1つ提案してください。質問せずに即座に提案してください。` +
                        `例: 【返信案】こんにちは！素敵な商品ですね。購入を検討しています。` +
                        `「なんでもいい」と言われたら挨拶の返信案を提案してください。`,
                },
            });
        }
